...
def n_fold_split_cluster_trait_wesad(subject_ids, n, dataset_name, seed=5):
    result = []
    ...
    for test_set in test_sets:
        ...
        FINAL = pd.read_csv(f'./archives/MTL/WESAD/Feature/dataset_test_{test_set}.csv', sep=",", index_col=0)
        FINAL['Cluster'] = np.where(FINAL['pnum'].isin(subject_ids_male), 0, 1)
        FINAL.to_csv(f'./archives/MTL/WESAD/Trait/dataset_test_{test_set}.csv', sep=",", index=False)
...
def n_fold_split_cluster_feature_wesad(subject_ids, n, seed=5):
    test_sets = [subject_ids[i::n] for i in range(n)]
...
# def n_fold_split_mtl_cluster_trait_wesad(subject_ids, n, dataset_name, seed=5):
#     result = []
