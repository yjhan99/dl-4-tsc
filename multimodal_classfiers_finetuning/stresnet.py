...
class ClassifierStresnet(Classifier):
    ...
    def __init__(self, output_directory, input_shapes, sampling_rates, ndft_arr, nb_classes, verbose=False,
    ...
    def build_model(self, input_shapes, nb_classes, hyperparameters):
        ...
        num_res_blocks = hyperparameters.depth if hyperparameters and hyperparameters.depth else 4
        cnn_per_res = 3
        ...
        x = keras.layers.Dense(16, activation="relu", kernel_regularizer=keras.regularizers.l2(l2_lambda))(x)
        x = keras.layers.BatchNormalization()(x)
        x = keras.layers.Dropout(drop_rate)(x)
        x = keras.layers.Dense(32, activation="relu", kernel_regularizer=keras.regularizers.l2(l2_lambda))(x)
        x = keras.layers.Dropout(drop_rate)(x)
