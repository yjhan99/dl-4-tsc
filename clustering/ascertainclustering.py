...
def n_fold_split_cluster_trait(subject_ids, n, dataset_name, seed=5):
    ...
    for test_subject in test_sets:
        ...
        file_path = "./archives/{0}/Personality.csv".format(dataset_name)
        ...
        X_rest = X.loc[X['pnum']!=str(test_subject[0])]
...
def n_fold_split_cluster_trait_mtl(subject_ids, n, dataset_name, seed=5):
    ...
    for test_subject in test_sets:
        ...
        # User-as-task
        X_test_scaled.columns = ['ex', 'ag', 'co', 'em']
        X_rest_scaled.columns = ['ex', 'ag', 'co', 'em']
        def calculate_distance(row, target):
            return distance.euclidean((row['ex'], row['ag'], row['co'], row['em']), (target['ex'], target['ag'], target['co'], target['em']))
