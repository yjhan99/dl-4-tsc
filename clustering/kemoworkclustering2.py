...
def n_fold_split_cluster_feature(subject_ids, n, seed=5):
    ...
    for test_subject in test_sets:
        ...
        dataset = Dataset("KEmoWork2", None, GLOBAL_LOGGER)
        ...
        try: 
            file_path = "./encodedresults/KEmoWork2/encoded_results_restof_{1}.csv".format("KEmoWork2",test_subject)
            file = pd.read_csv(file_path, usecols=lambda column: column != 'Unnamed: 0')
            X_encoded_df = pd.DataFrame(file)
            file_path = "./encodedresults/KEmoWork2/encoded_results_{1}.csv".format("KEmoWork2",test_subject)
            file = pd.read_csv(file_path, usecols=lambda column: column != 'Unnamed: 0')
                ...
                with session.as_default():
                    with tf.device('/device:GPU:0'):
                        ...
                        for channel_id, input_shape in enumerate(input_shapes):
                            ...
                            training_losses[channel_id] = history.history['loss']
            ...
            X_encoded_df.to_csv(f'./encodedresults/KEmoWork2/encoded_results_restof_{test_subject}.csv', sep=',')
            test_X_encoded_df.to_csv(f'./encodedresults/KEmoWork2/encoded_results_{test_subject}.csv', sep=',')

            with open(f'./encoderloss/KEmoWork2/training_losses_{test_subject}.txt', 'w') as file:
                for channel, losses in training_losses.items():
