...
def n_fold_split_cluster_feature(subject_ids, n, seed=5):
    ...
    for test_subject in test_sets:
        ...
        try: 
                ...
                with session.as_default():
                    with tf.device('/device:GPU:0'):
                        ...
                        for channel_id, input_shape in enumerate(input_shapes):
                            ...
                            val_losses[channel_id] = history.history['val_loss']
            ...
            with open(f'./encoderloss/WESAD/[{test_subject}]_training_losses.txt', 'w') as file:
            ...
            with open(f'./encodervalloss/WESAD/[{test_subject}]_val_losses.txt', 'w') as file:
                for channel, losses in val_losses.items():
                    file.write(f'Channel {channel}:\n')
                    for loss in losses:
                        file.write(str(loss) + '\n')
                    file.write('\n')
...
def n_fold_split_cluster_feature_mtl(subject_ids, n, seed=5):
    test_sets = [subject_ids[i::n] for i in range(n)]
    ...
    for test_subject in test_sets:
        ...
        result.append({"cluster": same_cluster, "test": test_subject})
